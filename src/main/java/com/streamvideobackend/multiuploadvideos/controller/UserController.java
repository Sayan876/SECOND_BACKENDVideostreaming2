...
public class UserController {
	...
	@PostMapping("/user")
	public ResponseEntity<ApiResponse1> postUser(@RequestParam String name, @RequestParam String email,
		...
		try {
			...
			return ResponseEntity.ok(new ApiResponse1(true, "User created successfully", user));

		} catch (RuntimeException e) {
			return ResponseEntity.badRequest().body(new ApiResponse1(false, e.getMessage(), null));

		} catch (Exception e) {
			return ResponseEntity.status(500).body(new ApiResponse1(false, "Something went wrong", null));
		}
	}
	...
	public ResponseEntity<User> updateProfileImage(
	    ...
	    User user = userService.updateUserProfileImage(email, profilePic);

	    return ResponseEntity.ok(user);
	}

//	@PatchMapping("/user/profile-pic")
//	public ResponseEntity<User> updateProfileImage(@RequestParam MultipartFile profilePic, Authentication auth) {
//		try {
//			String email = auth.getName();
//			User user = userService.findByEmail(email);
//			User updatedUser = userService.updateUserProfileImage(user.getId(), profilePic);
...
//    }
	...
	@GetMapping("/user/me")
	public Optional<User> getMyProfile(Authentication auth) {
		String email = auth.getName();
